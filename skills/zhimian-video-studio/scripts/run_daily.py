...
def _draft_scenes(topic: dict[str, str | int]) -> list[dict]:
    title = str(topic["title"])
    return [
        {"id": "hook", "narration": f"面试官问：{title}", "on_screen_text": title, "caption": title, "visual_type": "editorial", "source_refs": ["S1"]},
        {"id": "intuition", "narration": "先给一个直觉，再解释底层原因。", "on_screen_text": "先给直觉，再讲原理", "caption": "先给一个直觉，再解释底层原因。", "visual_type": "process", "visual_payload": {"steps": ["问题", "直觉", "原理"]}, "source_refs": ["S1"]},
        {"id": "principle", "narration": "把概念拆成输入、过程和输出。", "on_screen_text": "输入 → 过程 → 输出", "caption": "把概念拆成输入、过程和输出。", "visual_type": "process", "visual_payload": {"steps": ["输入", "过程", "输出"]}, "source_refs": ["S1"]},
        {"id": "followups", "narration": "真正拉开差距的是连续追问。", "on_screen_text": "追问链决定上限", "caption": "真正拉开差距的是连续追问。", "visual_type": "code", "visual_payload": {"code": "Q1: 为什么这样设计？\nQ2: 复杂度是多少？\nQ3: 边界情况呢？"}, "source_refs": ["S1"]},
        {"id": "answer", "narration": "高分回答要按结论、原理、取舍和落地四步说。", "on_screen_text": "结论 · 原理 · 取舍 · 落地", "caption": "高分回答要按结论、原理、取舍和落地四步说。", "visual_type": "process", "visual_payload": {"steps": ["结论", "原理", "取舍", "落地"]}, "source_refs": ["S1"]},
        {"id": "mistake", "narration": "不要背标准答案，要解释为什么。", "on_screen_text": "别背答案，讲为什么", "caption": "不要背标准答案，要解释为什么。", "visual_type": "editorial", "source_refs": ["S1"]},
    ]
...
def run(args: argparse.Namespace) -> Path:
    ...
    topic = topic_for_day(date.fromisoformat(run_date).day)
