...
7. **Design each scene in HyperFrames first (MANDATORY).** Author 6 standalone HyperFrames compositions at `batch-runs/<batch>/<date>/hyperframes/scene-N-<id>/index.html` (1080×1920, `data-duration="8"`). Run `npx hyperframes lint` — must be 0 errors. Then `npx hyperframes snapshot` at 1.5s/4s/7s and present to the user for per-scene selection. **Do not write Remotion component code until the user explicitly confirms each scene.** After confirmation, port 1:1 to Remotion components per the port rules in `references/hyperframes-motion.md`. Back up HyperFrames sources to `batch-runs/_hyperframes_backup/`. This step is the design source of truth — never skip straight to Remotion.
...
12. Run QA, including preview frames from the opening, generated-image scenes, middle scenes, and ending. 质量检查失败不得宣称成功.
13. Deliver links to video, cover, audio, generated images, image plan, copy, sources, timeline, and QA report for review.
...
## Hand-crafted scenes rendering

When scenes are designed by hand (benchmark episodes, batch re-rendering, or user-edited `scenes.json`), do not let `run_daily.py`'s `_draft_scenes` overwrite them. Use the standalone renderer `batch-runs/YYYY-MM-DD_to_31/render_episode.py`, which reads a pre-written `scenes.json` and runs the full audio-driven pipeline (VoxCPM2 → timeline → Remotion) without redrafting. The benchmark episode 2026-07-08「DP/DDP/ZeRO」was produced this way and is the reference for the per-scene motion playbook in `references/hyperframes-motion.md`.
...
# Render from a hand-crafted scenes.json (benchmark / batch re-render)
python batch-runs/2026-07-01_to_31/render_episode.py --date 2026-07-08
```
