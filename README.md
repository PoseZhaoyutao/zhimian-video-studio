...
| `skills/zhimian-video-studio/references/content-calendar.md` | 首月 30 天选题日历。 |
| `skills/zhimian-video-studio/scripts/run_daily.py` | 日更/手动生产 CLI。 |
| `skills/zhimian-video-studio/scripts/zhimian/vox.py` | VoxCPM2 适配器。 |
| `remotion/` | React/Remotion 竖屏视频与封面模板。 |
...
安装后，在 Codex 里说“调用智面引擎”“生成今天的视频”“重做今天的视频”，Skill 会按 `SKILL.md` 的生产流程执行。因为 Remotion 工程在仓库根目录，建议长期把 `ZHIMIAN_REPO_ROOT` 写进 PowerShell profile 或系统环境变量。
